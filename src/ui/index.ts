...
class RoborockUiServer extends HomebridgePluginUiServer {
  constructor() {
    super();
  ...
  private async getClientId(): Promise<string> {
    ...
    if (storagePath) {
      ...
      fs.writeFileSync(clientIdPath, JSON.stringify({ val: clientId, ack: true }, null, 2), 'utf8');
      return clientId;
    }
  ...
  private async sendTwoFactorEmail(payload: { email?: string; baseURL?: string }) {
    ...
    try {
      ...
      return { ok: false, message: error?.message || 'Failed to send verification email.' };
    }
  }

  private async verifyTwoFactorCode(payload: { email?: string; code: string; baseURL?: string }) {
    const email = payload.email;
    if (!email) {
      return { ok: false, message: 'Email is required.' };
    }
    ...
    try {
      ...
      console.error('2FA verification request failed:', error?.message || error);
  ...
  private async loginWithPassword(payload: { email?: string; password?: string; baseURL?: string }) {
    const email = payload.email;
    ...
    return { ok: false, message: loginResult?.msg || 'Login failed. Check your credentials.' };
  }

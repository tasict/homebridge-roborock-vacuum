...
class Roborock {
  ...
  getScenesForDevice(duid) {
    ...
    try {
        ...
        if (scene.param) {
          try {
            ...
            if (param.action && param.action.items) {
              ...
              const hasMatchingDevice = param.action.items.some(
                (item) => item.entityId === duid
              );
            ...
            this.log.warn(
              `Failed to parse scene param for scene ${scene.id}: ${parseError.message}`
            );
          }
        }
      }
      ...
      this.log.error(
        `Failed to filter scenes for device ${duid}: ${error.message}`
      );
      return [];
    }
  }
    ...
    for (const device of devices) {
      ...
      for (const [attribute, value] of Object.entries(deviceStatus)) {
        const targetConsumable = await this.getObjectAsync(
          `Devices.${duid}.consumables.${attribute}`
        );

        if (targetConsumable) {
          const val = value >= 0 && value <= 100 ? parseInt(value) : 0;
          await this.setStateAsync(`Devices.${duid}.consumables.${attribute}`, {
            val: val,
            ack: true,
          });
        }

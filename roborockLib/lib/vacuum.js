  ...
  async getParameter(duid, parameter, attribute) {
    ...
    try {
      if (parameter == "get_network_info") {
        ...
        const roomMqttCfg =
          this.adapter.config && this.adapter.config.currentRoomMqtt;
        if (roomMqttCfg && roomMqttCfg.enabled) {
          ...
          const interval =
            vac && vac._lastIsCleaning
              ? Math.min(fastInterval, idleInterval)
              : idleInterval;
          if (seconds % interval == 0 && !(vac && vac._roomPollInFlight)) {
            if (vac) vac._roomPollInFlight = true;
            try {
              const ds = await this.adapter.messageQueueHandler.sendRequest(
                duid,
                "get_prop",
                ["get_status"]
              );
              const s0 = (ds && ds[0]) || {};
              if (vac)
                vac._lastIsCleaning = this.adapter.isCleaning(s0["state"]);
              ...
              this.adapter.log.debug(
                `current_room poll failed: ${e && e.message}`
              );
            } finally {
          ...
          for (const attribute in deviceStatus[0]) {
            ...
            switch (attribute) {
              ...
              case "map_status": {
                ...
                // The map updater of the original ioBroker adapter does not exist
                // in this plugin, so no start/stop calls here.
                deviceStatus[0][attribute] =
                  deviceStatus[0][attribute] >> 2 ?? -1;
        ...
        if (mappedRooms.length < 1) {
          this.adapter.log.warn(
            `Failed to map rooms. You need to name your rooms via the mobile app on your phone.`
          );
        } else {
          for (const mappedRoom of mappedRooms) {
            ...
            if (roomName) {
              ...
              if (!this.adapter.segmentRoomNames[duid])
                this.adapter.segmentRoomNames[duid] = {};
    ...
    try {
      ...
      for (const cleaningAttribute in cleaningAttributes) {
        const mappedAttribute =
          mappedCleanSummary[cleaningAttribute] || cleaningAttribute;

        if (
          ["clean_time", "clean_area", "clean_count"].includes(mappedAttribute)
        ) {
          await this.adapter.setStateAsync(
            `Devices.${duid}.cleaningInfo.${cleaningAttribute}`,
            {
              val: this.calculateCleaningValue(
                mappedAttribute,
          ...
          for (const cleaningRecord in cleaningAttributes[cleaningAttribute]) {
            ...
            for (const cleaningRecordAttribute in cleaningRecordAttributes) {
              const mappedRecordAttribute =
                mappedCleaningRecordAttribute[cleaningRecordAttribute] ||
                cleaningRecordAttribute;
              await this.adapter.setStateAsync(
                `Devices.${duid}.cleaningInfo.records.${cleaningRecord}.${mappedRecordAttribute}`,
                {
                  val: this.calculateRecordValue(
                    mappedRecordAttribute,
                    cleaningRecordAttributes[cleaningRecordAttribute]
                  ),
                  ack: true,
                }
              );
          ...
          this.adapter.setStateAsync(`Devices.${duid}.cleaningInfo.JSON`, {
            val: JSON.stringify(cleaningRecordsJSON),
            ack: true,
          });
        }
      }
